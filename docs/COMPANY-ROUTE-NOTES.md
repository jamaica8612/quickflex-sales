...
- 구역노트 첫 화면은 전체 지도다. 검색 막대와 필터를 지도 위에 띄우고, 검색 결과는 `구역`·`메모` 뱃지가 붙은 목록으로 지도 위에 편다. `내 위치`와 `구역 만들기`는 지도 오른쪽 아래 아이콘 버튼이다. 구역 경계/이름을 선택하면 하단 시트에서 팁·사진·공유를 연다. 작은 화면의 시트는 손잡이를 끌거나 짧게 눌러 접힘·절반·최대 세 단계로 조절하고(키보드는 ↑↓), 넓은 화면은 우측 고정 패널이다. 앱 내비게이션 영역과 지도 레이어는 분리한다. 네이버 기본 확대/축소 버튼은 숨기고 지도 터치 제스처는 유지한다. 메모 검색은 열려 있는 구역의 메모까지 닿는다. 회사 전체 팁을 한 번에 내려받지 않기 때문이다.
