...
- Validation: `npm ci` completed, `node --check` covered 62 files with 0 errors, and the full Node suite passed 514/514 in this checkout. The expected legacy `expense-privacy-sql` failure did not reproduce here; no unexpected test failure was found. New tests cover the legacy storage cleanup, the removal migration (PGlite) and the notice copy. Mock Noah screen captured at 390px.
- Deployment record: migration applied only at approximately 2026-09-24 18:58 KST (provider version `20260924095751`); `noah` Edge Function v9 deployed at 2026-09-24 18:58:39 KST with `verify_jwt=true`; PR #6 merged to main as `046e5ceee8ab84bb68c52c3d20f3d833ba846086`; Pages run `35984530784` completed successfully. SQL verification found no `quickflex_noah_private.feedback` table or `public.quickflex_noah_submit_feedback` function, while `public.quickflex_noah_acknowledge_notice` and `public.quickflex_noah_consume_quota` remained.
- Live verification: manifest reported 1.0.97, privacy had no TODO and `#noah` landed on the OpenAI item, the notice showed the provider-free copy with `자세히 보기` to `privacy.html#noah`, and Noah had no feedback buttons or `오늘의 기록` card. The scroll hotfix changed the Noah stylesheet to `?v=4`, added contained vertical scrolling and pull-to-refresh protection, and bumped the shell cache to `quickflex-shell-v1.0.97-usage-guide-1-noah-scroll-1`; Pages run `35985297498` completed successfully and the live files contained the new rules.

## 2026-09-25 - Noah messenger layout and thinking bubble (PWA 1.0.98)

- User report: Noah bubbles and the text input were hard to see. Cause: in the light theme the user bubble (`--panel3 #F4F4F5`) and input (`--panel2 #F7F7F8`) were almost the page background (`#F5F5F5`); in the dark theme the Noah bubble (`#1C1C1C`) sat on `#141414`.
- Messenger layout (시안 B): user bubbles are filled with the accent (`--gold`), Noah bubbles are bordered cards (`#232323` in dark), the composer is a bordered pill inside a bottom bar with the send button inside, suggestions are a single horizontally scrolling row, and `새 대화` moved into the header next to settings.
- Thinking bubble: server `progress` events render as steps inside Noah's bubble (✓ done / spinner now, dedupe, "…했어요" closes the current step). The first answer delta folds it into a `<details>` summary such as `매출 기록 확인함 · 2.1초`. Chips hide and the old status line is screen-reader-only while pending.
...
## 2026-09-26 - Design review fixes and Beta 1.27 links (PWA 1.0.101)

- Review fixes 1-10 and 12 (11 left as is at the user's request, 13 keeps the 매출노트 tab name): the record-draft leave check compares normalized drafts; the record header back button is separate from the date arrows; the delivery bridge says so on a day off; the summary shows the real difference; 지출 follows the settlement period and its header says "지출"; dates read "9월 18일 (목)" / "9/18"; delivered items count in 건; 구역노트 follows the app theme, gets the shared tab header, splits glued zone codes for display ("302A · 303D"), and nudges zone labels off tip pins on the map (label 14px).
- Point download and release links to Android Beta 1.27 (versionCode 149, APK SHA-256 `fde931848faa4197047161f6d09f61c58f0b1dbe9fb0e1c2625c666fc886de32`) and advance the manifest, shell cache and entry URLs to 1.0.101. Changed RouteNote assets move to `route-notes.js?v=3` (lib and services), `ui/route-notes.js?v=16`, `route-note-map.js?v=10`, `route-note-map-geometry.js?v=3`, `route-note-zone-editor.js?v=8`, `route-notes.css?v=10`, `route-note-map.css?v=7`, `route-note-zone-editor.css?v=3`.
- No database or Edge Function change in this release; `quickflex_work_handoffs` (used by Beta 1.27) was applied with d875050.
- Validation: full `tests/*.test.mjs` 464/476 pass; the 12 failures all fail on the d875050 baseline as well (one baseline failure now passes). Map label placement was checked with a stubbed Naver SDK harness; the live Naver map only loads on the production domain.

## 2026-09-26 - Usage stats fix, sign-up notice, first-day period card, test health (PWA 1.0.102)

- Usage stats had never been stored: `quickflex_track_usage_event` runs as the caller, and `on conflict (user_id, client_event_id)` needs SELECT on those columns, which `authenticated` does not have, so every call failed with 42501 (403) since 2026-08-27. The function now uses an untargeted `on conflict do nothing`; the unique constraint still de-duplicates and no privilege was widened. The table check and the function now accept the `noah` and `routes` screens, and the client sends them.
- Admin-only RPCs `quickflex_usage_screen_summary(p_window_days)` and `quickflex_pending_signups()` check admin themselves (SECURITY DEFINER, empty search_path). The private-schema pattern was dropped because 20260901110218 revoked private schema usage from `authenticated`, which also leaves the older `quickflex_usage_summary` unusable.
- Settings gets an admin card "최근 30일 화면별 사용" (7일/30일). Admins see "가입 승인 대기 N명 · 가장 오래 D일째" on 매출노트 and a red dot on the gear; tapping opens the member list.
- On a new settlement period with no recorded work day, the 매출노트 headline card shows the period that just ended ("지난 정산 · 8/26 - 9/25") with a note that the new period starts with the first record, and 정산노트 defaults to "지난 정산" unless the user picked a range.
- Shared-project hardening (user approved): trigger-only SECURITY DEFINER functions of growing and RouteNote lost API EXECUTE (triggers still fire; checked in a rolled-back probe), and six functions got a fixed search_path (growing vector search checked with a narrowed caller search_path). `rn_is_admin` keeps EXECUTE because rn_* RLS policies call it. Leaked-password protection needs the Pro plan.
- Test health: `npm ci` brings in pglite for the SQL tests, one test lost a hard-coded path from another checkout, one became CRLF-safe. `scripts/release.mjs` bumps the PWA/Android versions (`--pwa`, `--android`), checks `?v=` assets against `release/asset-versions.json` (`--check`) and cascades bumps (`--bump-assets`).
- Validation: `node --test tests/*.test.mjs` 597/597 pass; `node scripts/release.mjs --check` passes. Migrations 20260926100000, 20260926103000 and 20260926104000 were applied to the live project; each was verified in rolled-back transactions as the owner account (noah/routes recorded, bogus screen rejected, summary and pending count returned). Security advisors: mutable search_path 6 -> 0, anon-executable SECURITY DEFINER 6 -> 1.
