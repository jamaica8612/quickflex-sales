...
const fmtWon = (n) => `${fmt.format(Math.round(Number(n) || 0))}원`;
const WEEKDAYS = ["일", "월", "화", "수", "목", "금", "토"];
const GOAL = 6_000_000;
const DB_KEY = "quickflex-supabase-config";
const OWNER_ID = "kim-gwanhyun";
const OWNER_NAME = "김관현";
const TABLES = {
  legacy: "quickflex_data",
...
const el = {
  ...
  statsBestAmount: $("statsBestAmount"),
  deltaWorkDays: $("deltaWorkDays"),
  deltaOffDays: $("deltaOffDays"),
  deltaCount: $("deltaCount"),
  deltaFresh: $("deltaFresh"),
...
function periodKeys() {
  return periodKeysFor(state.year, state.month);
}
...
function emptyRecord() {
  return { off: false, rows: [], freshCount: "", freshUnit: 100 };
}
function cloneRecord(record) {
  return {
    ...
    freshUnit: record?.freshUnit ?? 100,
  };
}
function normalizeRecordShape(record) {
  const next = cloneRecord(record || emptyRecord());
  next.rows = next.rows
    .map((row) => ({
  ...
  next.rows.forEach((row) => {
    if (!toNum(row.unit) && row.route) row.unit = rateFor(row.route);
  });
  next.freshUnit = next.freshUnit == null || next.freshUnit === "" ? 100 : next.freshUnit;
  next.off = Boolean(next.off);
  return next;
}
function getRecord(dateKey, create = false) {
  if (!state.entries[dateKey]) {
    if (!create) return emptyRecord();
    state.entries[dateKey] = emptyRecord();
  }
  state.entries[dateKey] = normalizeRecordShape(state.entries[dateKey]);
  return state.entries[dateKey];
}
function setRecord(dateKey, record) {
  state.entries[dateKey] = normalizeRecordShape(record);
}
function hasMeaningfulRecord(record) {
  const rec = normalizeRecordShape(record);
  return rec.off || rec.rows.length > 0 || toNum(rec.freshCount) > 0 || toNum(rec.freshUnit) !== 100;
}
...
function effectiveUnit(row) {
  ...
  const fallback = rateFor(row.route);
  if (fallback > 0) {
    row.unit = fallback;
    return fallback;
  }
...
function calcRecord(record) {
  ...
  const freshUnit = toNum(rec.freshUnit) || 100;
  return {
    count: routeTotal.count,
    revenue: routeTotal.revenue + freshCount * freshUnit,
  };
}
function summarizePeriod(year = state.year, month = state.month) {
  ...
  const total = periodKeysFor(year, month).reduce((sum, dateKey) => {
    ...
    const fresh = record.off ? 0 : toNum(record.freshCount);
...
function setSyncStatus(status) {
  ...
  el.syncIndicator.title = status === "saving" ? "DB 저장 중" : status === "error" ? "DB 저장 실패" : "DB 연결됨";
}
...
function hydrateState(rates, entries) {
...
async function persistRates() {
  ...
  const payload = state.rates
    .filter((rate) => normalizeRoute(rate.route) && toNum(rate.unit) > 0)
    .map((rate) => ({
      user_id: OWNER_ID,
  ...
  const { error } = await state.db.from(TABLES.rates).insert(payload);
...
async function persistDay(dateKey) {
  ...
  const { error: deleteItemsError } = await state.db
    .from(TABLES.items)
    .delete()
    .eq("user_id", OWNER_ID)
  ...
  if (!hasMeaningfulRecord(rec)) {
    const { error: deleteDayError } = await state.db
      .from(TABLES.days)
      .delete()
      .eq("user_id", OWNER_ID)
      .eq("work_date", dateKey);
  ...
  const { error: dayError } = await state.db.from(TABLES.days).upsert({
    user_id: OWNER_ID,
    work_date: dateKey,
    is_off: Boolean(rec.off),
    fresh_count: toNum(rec.freshCount),
    fresh_unit: toNum(rec.freshUnit) || 100,
    updated_at: new Date().toISOString(),
  }, { onConflict: "user_id,work_date" });
  if (dayError) throw dayError;

  const payload = rec.rows
    .map((row, index) => ({
      user_id: OWNER_ID,
      work_date: dateKey,
      route: normalizeRoute(row.route),
      delivery_count: toNum(row.count),
      unit_snapshot: toNum(row.unit) || rateFor(row.route),
      sort_order: index,
      updated_at: new Date().toISOString(),
    }))
  ...
  if (!payload.length) return;
  const { error: itemError } = await state.db.from(TABLES.items).insert(payload);
  if (itemError) throw itemError;
}
async function migrateLegacyBlobIfNeeded(rateRows, dayRows, itemRows) {
  if ((rateRows?.length || 0) > 0 || (dayRows?.length || 0) > 0 || (itemRows?.length || 0) > 0) return false;
  ...
  const migratedRates = Array.isArray(data.rates) ? data.rates.map((rate) => ({
    route: normalizeRoute(rate.route),
    unit: toNum(rate.unit),
    count: 0,
    amount: 0,
  })) : [];
  const migratedEntries = {};
  Object.entries(data.entries || {}).forEach(([dateKey, record]) => {
    migratedEntries[dateKey] = normalizeRecordShape(record);
  });
  hydrateState(migratedRates, migratedEntries);

  await persistRates();
  for (const dateKey of Object.keys(state.entries)) {
    await persistDay(dateKey);
  }

  setDbBadge(true, new Date(data.updated_at || Date.now()));
  toast("기존 데이터를 새 DB 구조로 옮겼습니다.", "success");
  return true;
}
async function loadFromDb() {
  if (!state.db) return false;
  try {
    const [rateRes, dayRes, itemRes] = await Promise.all([
      state.db.from(TABLES.rates).select("route,current_unit").eq("user_id", OWNER_ID).order("route"),
      state.db.from(TABLES.days).select("work_date,is_off,fresh_count,fresh_unit,updated_at").eq("user_id", OWNER_ID).order("work_date"),
      state.db.from(TABLES.items).select("work_date,route,delivery_count,unit_snapshot,sort_order").eq("user_id", OWNER_ID).order("work_date").order("sort_order"),
    ]);
    if (rateRes.error) throw rateRes.error;
    if (dayRes.error) throw dayRes.error;
    if (itemRes.error) throw itemRes.error;

    const migrated = await migrateLegacyBlobIfNeeded(rateRes.data, dayRes.data, itemRes.data);
    if (migrated) return loadFromDb();

    hydrateState(ratesFromDb(rateRes.data), entriesFromDb(dayRes.data, itemRes.data));
    state.synced = true;
    const latestUpdatedAt = dayRes.data?.length
      ? new Date(dayRes.data.reduce((latest, row) => row.updated_at > latest ? row.updated_at : latest, dayRes.data[0].updated_at))
      : new Date();
    setDbBadge(true, latestUpdatedAt);
    setSyncStatus("idle");
    return true;
  } catch (error) {
    ...
    setSyncStatus("error");
    setDbBadge(false);
    toast(`DB 불러오기 실패: ${error.message}`, "error");
    return false;
  }
}
function scheduleSave(options = {}) {
  const { dateKeys = [], rates = false, immediate = false } = options;
  dateKeys.forEach((dateKey) => state.pendingDates.add(dateKey));
  ...
  state.saveTimer = setTimeout(flushPendingSaves, 450);
}
async function flushPendingSaves() {
  ...
  const pendingDates = [...state.pendingDates];
  const pendingRates = state.pendingRates;
  if (!pendingDates.length && !pendingRates) return;

  state.flushing = true;
  setSyncStatus("saving");
  try {
    if (pendingRates) await persistRates();
    for (const dateKey of pendingDates) {
      await persistDay(dateKey);
    }
    if (pendingRates) state.pendingRates = false;
    pendingDates.forEach((dateKey) => state.pendingDates.delete(dateKey));
    setSyncStatus("idle");
    setDbBadge(true, new Date());
  } catch (error) {
...
async function connectDb(url, key, showSuccessToast = true) {
  const client = buildClient(url, key);
  if (!client) throw new Error("Supabase 연결 객체를 만들 수 없습니다.");

  const [rateCheck, dayCheck] = await Promise.all([
    client.from(TABLES.rates).select("route").eq("user_id", OWNER_ID).limit(1),
    client.from(TABLES.days).select("work_date").eq("user_id", OWNER_ID).limit(1),
  ]);
  if (rateCheck.error) throw rateCheck.error;
  if (dayCheck.error) throw dayCheck.error;

  saveDbConfig(url, key);
  state.db = client;
  const ok = await loadFromDb();
  if (!ok) throw new Error("DB 동기화에 실패했습니다.");

  el.supabaseUrl.value = url;
  el.supabaseAnonKey.value = key;
  hideSetupOverlay();
  renderAll();
  if (showSuccessToast) toast("DB 연결 완료", "success");
}
async function init() {
  setDbBadge(false);
  setSyncStatus("error");
  if (el.setupSkip) el.setupSkip.style.display = "none";
  renderAll();
...
function renderSummary() {
  ...
  el.periodRevenue.textContent = fmtWon(summary.revenue);
  el.periodCount.textContent = `${fmt.format(summary.count)}건`;
  el.dailyAverage.textContent = fmtWon(summary.average);
  el.workDaysHome.textContent = `${summary.workDays}일`;
  el.meterFill.style.width = `${Math.min((summary.revenue / GOAL) * 100, 100)}%`;
}
function renderMonth() {
  ...
  el.periodRangeShort.textContent = `${shortDate(start)} ~ ${shortDate(end)}`;
  ...
  while (cursor <= gridEnd) {
    ...
    cell.className = [
      ...
      dateKey === nowKey ? "today-cell" : "",
    ].filter(Boolean).join(" ");
    const dayLabel = cursor.getDate() === 1 ? `${cursor.getMonth() + 1}/1` : String(cursor.getDate());
    let valueText = "";
    if (record.off) valueText = "휴무";
    else if (state.mode === "count" && calc.count > 0) valueText = `${fmt.format(calc.count)}건`;
    else if (state.mode === "amount" && calc.revenue > 0) valueText = fmt.format(calc.revenue);
    cell.innerHTML = `<span class="day-number">${dayLabel}</span><span class="day-value">${valueText}</span>`;
...
function renderEntryForm() {
  ...
  el.recordBody.classList.toggle("is-off", record.off);
  el.entryRows.innerHTML = "";

  if (record.rows.length === 0 && !record.off && state.rates.length) {
    record.rows.push({
      route: state.rates[0].route,
      count: "",
      unit: state.rates[0].unit,
    });
  }

  record.rows.forEach((row, index) => {
    const node = el.entryTemplate.content.firstElementChild.cloneNode(true);
    const routeSelect = node.querySelector("select");
    const countInput = node.querySelector(".count");
    const unitInput = node.querySelector(".unit");
    const amountOutput = node.querySelector("output");
    const deleteButton = node.querySelector(".del-btn");

    const routes = [...new Set([...state.rates.map((rate) => rate.route), normalizeRoute(row.route)].filter(Boolean))].sort();
    routeSelect.innerHTML = routes.map((route) => `<option value="${route}"${route === normalizeRoute(row.route) ? " selected" : ""}>${route}</option>`).join("");
    countInput.value = row.count ?? "";
    unitInput.value = effectiveUnit(row) || "";
    amountOutput.textContent = fmtWon(toNum(row.count) * effectiveUnit(row));

    routeSelect.addEventListener("change", () => {
      row.route = normalizeRoute(routeSelect.value);
      if (!toNum(row.unit)) row.unit = rateFor(row.route);
      renderEntryForm();
      scheduleSave({ dateKeys: [state.selectedDate] });
      refreshTotals();
    });
    countInput.addEventListener("input", () => {
      row.count = countInput.value;
      amountOutput.textContent = fmtWon(toNum(row.count) * effectiveUnit(row));
      scheduleSave({ dateKeys: [state.selectedDate] });
      refreshTotals();
    });
    unitInput.addEventListener("input", () => {
      ...
      amountOutput.textContent = fmtWon(toNum(row.count) * toNum(unitInput.value));
      scheduleSave({ dateKeys: [state.selectedDate] });
      refreshTotals();
    });
  ...
  el.freshRevenue.textContent = fmtWon(toNum(record.freshCount) * toNum(record.freshUnit || 100));
  el.selectedDayTotal.textContent = fmtWon(calcRecord(record).revenue);
}
function refreshTotals() {
  ...
  renderMonth();
  renderHomeSelection();
  if (el.app.dataset.view === "stats") renderStats();
}
function renderRates() {
  el.rateList.innerHTML = state.rates
    .map((rate) => `<div class="rate-chip" data-route="${rate.route}" data-unit="${rate.unit}"><strong>${rate.route}</strong><span>${fmt.format(rate.unit)}원</span></div>`)
    .join("");
...
function renderDailyList(year, month) {
  ...
  periodKeysFor(year, month).reverse().forEach((dateKey) => {
    const record = getRecord(dateKey, false);
    const calc = calcRecord(record);
    const item = document.createElement("div");
    item.className = `daily-item${record.off ? " off" : ""}`;
    item.innerHTML = `<span class="date">${formatLongShort(dateKey)}</span><span class="amt">${record.off ? "휴무" : fmtWon(calc.revenue)}</span>`;
    el.dailyList.appendChild(item);
  });
}
function renderRouteList(year, month) {
  const acc = new Map();
  periodKeysFor(year, month).forEach((dateKey) => {
    const record = getRecord(dateKey, false);
    ...
    record.rows.forEach((row) => {
      ...
      const count = toNum(row.count);
      const unit = effectiveUnit(row);
      const current = acc.get(route) || { count: 0, revenue: 0 };
      acc.set(route, { count: current.count + count, revenue: current.revenue + count * unit });
    });
  });
  ...
  [...acc.entries()]
    ...
    .forEach(([route, value]) => {
      ...
      item.innerHTML = `<span class="route-id">${route}</span><div class="route-detail"><span class="route-amt">${fmtWon(value.revenue)}</span><span class="route-count">${fmt.format(value.count)}건</span></div>`;
...
function renderStats() {
  ...
  const { start, end } = periodBounds(year, month);
  const current = summarizePeriod(year, month);
  const previous = prevPeriod(year, month);
  const previousSummary = summarizePeriod(previous.year, previous.month);
  ...
  el.statsRevenue.textContent = fmtWon(current.revenue);
  el.statsMeterFill.style.width = `${Math.min((current.revenue / GOAL) * 100, 100)}%`;
  el.statsWorkDays.textContent = `${current.workDays}일`;
  el.statsOffDays.textContent = `${current.offDays}일`;
  el.statsCount.textContent = `${fmt.format(current.count)}건`;
  el.statsFresh.textContent = `${fmt.format(current.fresh)}건`;
  el.statsAverage.textContent = fmtWon(current.average);
  el.statsBestDay.textContent = current.best.dateKey ? formatLongShort(current.best.dateKey) : "-";
  el.statsBestAmount.textContent = current.best.revenue ? fmtWon(current.best.revenue) : "";

  const setDelta = (node, currentValue, previousValue, unit) => {
    node.innerHTML = "";
    const diff = Math.round(currentValue - previousValue);
    if (!diff) return;
    const badge = document.createElement("em");
    badge.className = diff > 0 ? "up" : "dn";
    badge.textContent = `${fmt.format(Math.abs(diff))}${unit}${diff > 0 ? "↑" : "↓"}`;
    node.appendChild(badge);
  };
  setDelta(el.deltaWorkDays, current.workDays, previousSummary.workDays, "일");
  setDelta(el.deltaOffDays, current.offDays, previousSummary.offDays, "일");
  setDelta(el.deltaCount, current.count, previousSummary.count, "건");
  setDelta(el.deltaFresh, current.fresh, previousSummary.fresh, "건");
  setDelta(el.deltaAverage, current.average, previousSummary.average, "원");
...
function upsertRate(route, unit) {
  ...
  const normalizedUnit = toNum(unit);
  if (!normalizedRoute || normalizedUnit <= 0) return false;
  const existing = state.rates.find((rate) => rate.route === normalizedRoute);
  if (existing) existing.unit = normalizedUnit;
  else state.rates.push({ route: normalizedRoute, unit: normalizedUnit, count: 0, amount: 0 });
...
function parseSettlementCsv(text) {
  ...
  const header = splitLine(lines[0]).map((cell) => cell.replace(/\s/g, ""));
  ...
  if (routeIndex < 0 || countIndex < 0 || amountIndex < 0) {
    toast("Route, 배송건수, 금액 열을 찾지 못했습니다.", "error");
    return [];
  }
...
function parseHeaderDate(value) {
  ...
  const year = month > state.month ? state.year - 1 : state.year;
  return `${year}-${String(month).padStart(2, "0")}-${String(short[2]).padStart(2, "0")}`;
}
function routesFromCell(value) {
  const text = String(value || "").trim();
  if (!text || /휴무|off/i.test(text)) return null;
  const routes = text.split(/[,/\s]+/).map(normalizeRoute).filter(Boolean);
...
function applySchedule(dateMap) {
  ...
  scheduleSave({ dateKeys: changedDates, immediate: true });
  renderSummary();
  renderMonth();
  renderHomeSelection();
...
function parseScheduleCsv(text) {
  ...
  const header = rows[0];
  const target = rows.find((row, index) => index > 0 && row.some((cell) => String(cell).replace(/\s/g, "").includes(OWNER_NAME)));
  if (!target) {
    toast(`${OWNER_NAME} 행을 찾지 못했습니다.`, "error");
    return;
  }
  const map = {};
  header.forEach((cell, index) => {
    ...
    if (!dateKey) return;
    map[dateKey] = routesFromCell(target[index]);
  });
  applySchedule(map);
}
function routeTokensFromText(text) {
  ...
  tokens.forEach((token) => {
    if (token === "휴무" || token === "OFF") {
      groups.push(current.length ? current : []);
      current = [];
    } else {
      current.push(token);
    }
...
function dateKeysFromOcrText(text) {
  const matches = [...String(text || "").matchAll(/(\d{1,2})\s*[./]\s*(\d{1,2})/g)];
  const seen = new Set();
  return matches
...
function parseScheduleOcrText(text) {
  const lines = String(text || "").split(/\r?\n/).map((line) => line.trim()).filter(Boolean);
  const normalize = (value) => value.replace(/\s+/g, "").replace(/[^\u3131-\uD79D\w]/g, "");
  const targetLine = lines.find((line) => normalize(line).includes(OWNER_NAME));
  if (!targetLine) {
    toast(`${OWNER_NAME} 행을 찾지 못했습니다.`, "error");
    return;
  }
  const dateKeys = dateKeysFromOcrText(text);
  const routeGroups = routeTokensFromText(targetLine.replace(/김\s*관\s*현/g, ""));
  const keys = dateKeys.length ? dateKeys : periodKeys();
  const map = {};
  routeGroups.forEach((routes, index) => {
    const dateKey = keys[index];
    if (!dateKey) return;
    map[dateKey] = routes.length ? routes : null;
  });
  applySchedule(map);
}
...
el.setupConnect.addEventListener("click", async () => {
  ...
  if (!url || !key) {
    el.setupError.textContent = "Project URL과 anon key를 모두 입력해 주세요.";
    return;
  }
  el.setupConnect.disabled = true;
  try {
    await connectDb(url, key, true);
  } catch (error) {
...
el.setupSkip.addEventListener("click", () => {
  toast("이제는 DB 연결이 있어야 기록을 저장할 수 있습니다.", "error");
});
...
el.copySchedule.addEventListener("click", () => {
  ...
  let changed = false;
  record.rows.forEach((row) => {
    if (row.route && !toNum(row.unit)) {
      row.unit = rateFor(row.route);
      changed = true;
    }
  });
  if (!record.rows.length && state.rates.length) {
    record.rows.push({ route: state.rates[0].route, count: "", unit: state.rates[0].unit });
    changed = true;
  }
  if (!changed) {
    toast("현재 날짜에 채울 스케줄 정보가 없습니다.", "error");
    return;
  }
  scheduleSave({ dateKeys: [state.selectedDate] });
  renderEntryForm();
...
document.querySelectorAll(".mode-btn").forEach((button) => {
  button.addEventListener("click", () => {
    ...
    document.querySelectorAll(".mode-btn").forEach((target) => target.classList.toggle("active", target === button));
    renderMonth();
  });
});
...
el.parseCsv.addEventListener("click", () => {
  ...
  renderAll();
  scheduleSave({ rates: true, immediate: true });
  toast(`단가 ${state.rates.length}개를 갱신했습니다.`, "success");
});
el.resetData.addEventListener("click", async () => {
  if (!state.db) {
    toast("DB 연결 후에만 초기화할 수 있습니다.", "error");
    return;
  }
  if (!window.confirm("DB에 저장된 단가와 기록을 모두 초기화할까요?")) return;
  try {
    setSyncStatus("saving");
    await state.db.from(TABLES.items).delete().eq("user_id", OWNER_ID);
    await state.db.from(TABLES.days).delete().eq("user_id", OWNER_ID);
    await state.db.from(TABLES.rates).delete().eq("user_id", OWNER_ID);
    state.rates = [];
    state.entries = {};
...
el.saveDbConfig.addEventListener("click", async () => {
  ...
  if (!url || !key) {
    toast("Project URL과 anon key를 입력해 주세요.", "error");
    return;
  }
  try {
    await connectDb(url, key, true);
