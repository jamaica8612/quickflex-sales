...
test("PWA config names the immutable-receipt detail and date override contracts", () => {
  ...
  assert.match(serviceWorker, /quickflex-shell-v1\.0\.49/);
  assert.match(html, /src\/main\.js\?v=1\.0\.49/);
  assert.match(html, /styles\.css\?v=1\.0\.49/);
  assert.match(html, /퀵플렉스 매출관리 v1\.0\.49/);
  const parsedManifest = JSON.parse(manifest);
  assert.equal(parsedManifest.version, "1.0.49");
  assert.deepEqual(
