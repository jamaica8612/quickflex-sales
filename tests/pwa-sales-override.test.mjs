...
const routeShareHtml = readFileSync(new URL("../route-share.html", import.meta.url), "utf8");
...
test("PWA config names the immutable-receipt detail and date override contracts", () => {
  ...
  assert.equal(releaseVersion, "1.0.102");
  ...
  assert.ok(html.includes(`manifest.webmanifest?v=${releaseVersion}`));
  assert.ok(installHtml.includes(`manifest.webmanifest?v=${releaseVersion}`));
  ...
  assert.equal((html.match(/플렉스노트 · Beta 1.27/g) || []).length, 2);
  assert.match(html, /releases\/download\/android-beta-1.27\/flexnote-beta-1.27\.apk/);
  assert.match(installHtml, /releases\/download\/android-beta-1.27\/flexnote-beta-1.27\.apk/);
  assert.match(installHtml, /releases\/tag\/android-beta-1.27/);
  ...
  assert.equal(parsedManifest.short_name, "플렉스노트");
  assert.equal(parsedManifest.description, "배송의 기록부터 매출·지출까지");
  assert.ok(serviceWorker.includes(`manifest.webmanifest?v=${parsedManifest.version}`));
  assert.deepEqual(
