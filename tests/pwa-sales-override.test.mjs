...
test("PWA config names the immutable-receipt detail and date override contracts", () => {
  ...
  assert.equal(releaseVersion, "1.0.91");
  ...
  assert.equal((html.match(/플렉스노트 · Beta 1\.22/g) || []).length, 2);
  assert.match(html, /releases\/download\/android-beta-1\.22\/flexnote-beta-1\.22\.apk/);
  assert.match(installHtml, /releases\/download\/android-beta-1\.22\/flexnote-beta-1\.22\.apk/);
  assert.match(installHtml, /releases\/tag\/android-beta-1\.22/);
