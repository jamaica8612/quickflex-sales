...
test("PWA config names the immutable-receipt detail and date override contracts", () => {
  ...
  assert.equal(releaseVersion, "1.0.58");
  ...
  assert.equal((html.match(/플렉스노트 · Beta 1\.06/g) || []).length, 2);
  assert.match(html, /releases\/download\/android-beta-1\.06\/flexnote-beta-1\.06\.apk/);
