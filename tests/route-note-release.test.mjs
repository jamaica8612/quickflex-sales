...
test("new route editor exports and postcode service bypass pre-1.0.86 cached modules", async () => {
  ...
  const edges = [
    ...
    [read("src/ui/route-note-zone-editor.js"), "../lib/route-note-map.js?v=10", "./src/lib/route-note-map.js?v=10"],
    [read("route-share.js"), "./src/lib/route-note-map.js?v=10", "./src/lib/route-note-map.js?v=10"],
    ...
    [read("src/lib/route-note-map.js"), "./route-note-map-geometry.js?v=3", "./src/lib/route-note-map-geometry.js?v=3"],
    ...["src/services/route-notes.js", "src/ui/route-notes.js", "src/ui/route-note-zone-editor.js"].map((file) => [read(file), "../lib/route-notes.js?v=3", "./src/lib/route-notes.js?v=3"]),
  ];
  ...
  const rules = await import("../src/lib/route-notes.js?v=3");
  assert.equal(rules.routeNoteZoneNameKey("３１０ c"), "310C");
  const { createRouteNotesService } = await import("../src/services/route-notes.js?v=3");
  ...
  for (const file of ["index.html", "route-share.html"]) {
    assert.ok(read(file).includes("./styles/route-note-map.css?v=7"), file);
  }
  assert.ok(sw.includes('"./styles/route-note-map.css?v=7"'));
