...
test("all-day event keeps the supplied work date and leaves revenue off by default", () => {
  ...
  assert.equal(event.title, "퀵플렉스 근무");
...
test("revenue display is opt-in and can be removed while the work event remains", () => {
  ...
  assert.equal(buildCalendarDesiredEvent(day, { includeWork: true, includeOff: true, includeRoute: true, includeRevenue: true }).title, "퀵플렉스 근무 · 310C01 · 0원");
  assert.equal(buildCalendarDesiredEvent(day, { includeWork: true, includeOff: true, includeRoute: true, includeRevenue: false }).title, "퀵플렉스 근무 · 310C01");
});
...
test("revenue-only days distinguish missing amounts from a real zero", () => {
  ...
  assert.equal(zero.title, "퀵플렉스 매출 · 0원");
