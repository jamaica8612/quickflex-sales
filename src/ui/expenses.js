...
export function createExpensesController({host,getService,toast=()=>{}}) {
  ...
  function open(row=null,inbox=false) {
    ...
    dialog.innerHTML=`<form data-editor novalidate><header class="expense-dialog-head"><h2 id="expenseEditorTitle">${trashed?'휴지통 기록':inbox?'영수증 보관':row?'지출 수정':'새 지출'}</h2><button type="button" class="round-btn" data-close aria-label="지출 창 닫기">×</button></header><div class="expense-dialog-body">${trashed?'<p class="expense-trash-note">휴지통에 있는 기록입니다. 복원하기 전에는 고칠 수 없고 합계에도 들어가지 않습니다.</p>':''}${inbox?'<p class="expense-help">사진만 남기고 날짜와 금액은 나중에 채워도 됩니다.</p>':''}<div class="expense-file-actions"><button type="button" class="secondary-btn" data-camera>사진 촬영</button><button type="button" class="secondary-btn" data-upload>사진 · PDF 선택</button><input data-camera-input type="file" accept="image/jpeg,image/png,image/webp,image/heic" capture="environment" hidden /><input data-upload-input type="file" accept="image/jpeg,image/png,image/webp,image/heic,application/pdf" multiple hidden /></div><div data-files class="expense-files"></div><div class="expense-fields"><label class="expense-amount-field">총 금액 <span>원</span><input name="gross_amount" type="number" inputmode="numeric" min="0" step="1" value="${esc(draft.gross_amount)}" placeholder="금액 입력" /></label><label>지출 날짜<input name="actual_date" type="date" value="${esc(draft.actual_date)}" /></label><label>분류<select name="category">${options(CATEGORIES,draft.category)}</select></label><label class="expense-field-full">사용처<input name="merchant" maxlength="100" value="${esc(draft.merchant)}" placeholder="예: 주유소, 정비소" /></label></div><details class="expense-more"><summary>결제 · 세금 참고 정보</summary><div class="expense-fields"><label>결제수단<select name="payment_method">${options([['unknown','미입력'],['card','카드'],['cash','현금'],['transfer','계좌이체'],['other','기타']],draft.payment_method||'unknown')}</select></label><label>증빙 종류<select name="evidence_type">${options([['unknown','미확인'],['card','카드 영수증'],['cash','현금영수증'],['tax_invoice','세금계산서'],['receipt','일반 영수증'],['other','기타']],draft.evidence_type||'unknown')}</select></label><label>공급가액<input name="supply_amount" type="number" inputmode="numeric" min="0" step="1" value="${esc(draft.supply_amount)}" placeholder="확인한 경우 입력" /></label><label>부가세<input name="vat_amount" type="number" inputmode="numeric" min="0" step="1" value="${esc(draft.vat_amount)}" placeholder="확인한 경우 입력" /></label><label>사용 구분<select name="usage_type">${options([['business','업무용'],['personal','개인용'],['mixed','업무 · 개인 혼합']],draft.usage_type||'business')}</select></label><label>업무 사용 금액<input name="business_amount" type="number" inputmode="numeric" min="0" step="1" value="${esc(draft.business_amount)}" placeholder="확인한 경우 입력" /></label></div><p class="expense-help">공제 여부는 확정하지 않습니다. 확인하지 않은 세액은 비워두세요.</p></details><label class="expense-memo">메모<textarea name="memo" maxlength="1000" rows="2" placeholder="지출에 대해 남길 내용">${esc(draft.memo)}</textarea></label>${row&&!trashed&&draft.status==='confirmed'?`<details class="expense-more"><summary>환불 · 비용 보전 기록</summary><div data-adjustments>${(draft.adjustments||[]).map((a)=>`<p>${a.kind==='refund'?'환불':'비용 보전'} · ${esc(a.actual_date)} · ${money(a.amount)}원</p>`).join('')||'<p class="expense-help">환불이나 회사에서 돌려받은 금액을 따로 기록합니다.</p>'}</div><div class="expense-fields"><label>구분<select data-adjust-kind><option value="refund">환불</option><option value="reimbursement">비용 보전</option></select></label><label>금액<input data-adjust-amount type="number" inputmode="numeric" min="1" step="1" /></label><label>받은 날짜<input data-adjust-date type="date" value="${localDate()}" /></label><label>메모<input data-adjust-memo maxlength="500" /></label></div><button type="button" class="secondary-btn" data-adjust>추가</button></details>`:''}</div><p data-status class="expense-form-status" role="status" aria-live="polite"></p><footer class="expense-dialog-footer">${trashed?'<button type="button" class="full-btn" data-restore>지출 복원</button>':'<button type="button" class="secondary-btn" data-draft>임시 저장</button><button type="submit" class="full-btn">지출 저장</button>'}</footer>${row&&!trashed?'<button type="button" class="expense-trash" data-trash>휴지통으로 이동</button>':''}</form>`;
    ...
    dialog.addEventListener('change',(event)=>{
      ...
      for(const file of event.target.files||[]) {
        ...
        if(!ALLOWED_RECEIPT_TYPES.has(file.type)){report('JPEG, PNG, WebP, HEIC 사진 또는 PDF 파일을 선택해 주세요.');continue;}
  ...
  async function onDialogClick(event) {
    ...
    try {
      if(button.hasAttribute('data-preview')) {
        ...
        const link=document.createElement('button');link.type='button';link.dataset.openOriginal=String(button.dataset.preview);link.textContent='원본 열기';area.append(link);return;
      }
      if(button.hasAttribute('data-open-original')) {
        const receipt=draft.receipts[Number(button.dataset.openOriginal)];
        const opened=window.open('','_blank');
        if(opened) opened.opener=null;
        const service=await getService();const url=await service.receiptUrl(receipt);
        if(!alive(token)||dialog!==activeDialog){opened?.close();return;}
        if(!opened){report('팝업이 차단되었습니다. 브라우저에서 팝업을 허용한 뒤 다시 눌러 주세요.');return;}
        opened.location.replace(url);return;
      }
